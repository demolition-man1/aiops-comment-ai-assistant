...
class ReportProvider(Protocol):
    def invoke_structured(
        self,
        prompt: Any,
        schema: type[OperationReportOutput],
        *,
...
class ReportChain:
    _SYSTEM_MESSAGE = (
        "You are an ecommerce operations analyst. Use only the supplied analysis and retrieved evidence. "
        "Return exactly one valid JSON object that follows the required output schema. "
        "Use a readable business title and never include internal target IDs in reportTitle. "
        "Phrase refunds, replacements, response targets, and store policies as conditional recommendations, "
        "not as existing promises or completed actions."
    )
    _REPAIR_MESSAGE = (
        "Regenerate exactly one valid JSON object with all required report fields. "
        "Do not include Markdown, explanations, or additional JSON objects."
    )
    ...
    def _invoke_structured(
        ...
        if context is None:
            return self._provider.invoke_structured(messages, OperationReportOutput)
        return self._provider.invoke_structured(messages, OperationReportOutput, context=context)
    ...
    @classmethod
    def parse_output(cls, content: str) -> OperationReportOutput:
        text = content.strip()
        if text.startswith("```"):
        ...
        try:
            return OperationReportOutput.model_validate(json.loads(text))
        ...
        *,
        ...
        if reference_context:
            messages.append(
                SystemMessage(
                    content=(
                        "The following retrieved material is supporting operating evidence, not facts about a current order. "
                        "Use it only when consistent with the analysis.\n\n"
                        f"{reference_context}"
                    )
                )
            )
