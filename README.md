...
登录后进入“数据导入”页面，可以先点击“一键导入示例数据”快速生成可演示数据；上传单个 CSV 时，页面会先展示字段映射、预计导入行数和前 20 行预览，点击“开始导入”后才会上传到 OSS 并创建导入任务。完成导入后，可在“标签库”维护业务标签，在“方案库”维护问题处理方案，并回到“评论分析”页面查看推荐方案。
