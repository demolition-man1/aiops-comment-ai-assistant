...
评论工作台同时提供“仅分析评论”和“分析并生成报告”两个入口。完整流程由前端统一编排同一商品 ID，在分析任务成功并加载最新分析结果后才调用 AI 报告接口；如果报告生成失败，已完成的分析结果仍保留，商家可以单独重新生成报告。
...
```text
1. 用户点击“分析并生成报告”完成分析后自动进入报告生成，或在已有分析结果时单独点击生成 AI 运营报告。
...
- 查询商品分析结果。
- 查询卖家分析结果。
- 支持仅运行评论分析，或一键串行完成评论分析、结果加载和 AI 运营报告生成。
- 负面关键词排行。
- 主题聚类统计。
- 自定义标签分布统计。
- 按日、周、月汇总评论情感趋势。
